...
def load_in_file(file_path: str, colname: str):
    ...
    file_data[colname] = (
        file_data[colname].astype(str).str.replace("\bnan\b", "", regex=True)
    )
...
def data_file_to_markdown_table(
    ...
    simple_file["Response"] = simple_file["Response"].str.replace(
        r'[\x00-\x1F\x7F]|["“”‘’<>]|\\', "", regex=True
    )  # Remove control and special characters
    simple_file["Response"] = simple_file[
        "Response"
    ...
    simple_file = simple_file.loc[
        ~(simple_file["Response"].isnull()) & ~(simple_file["Response"].str.len() < 5),
        :,
    ]

    simplified_csv_table_path = (
        output_folder
        + "simple_markdown_table_"
        + file_name
        + "_row_"
        + str(start_row)
        + "_to_"
...
def call_aws_claude(
    ...
    response = ResponseObject(text=text, usage_metadata=request["ResponseMetadata"])
    ...
    if model_choice in GEMINI_MODELS:
        try:
            response = model.generate_content(
                contents=full_prompt, generation_config=config
            )
        except Exception as e:
...
def clean_markdown_table(text: str):
    ...
    header_pipes = merged_lines[0].count(
        "|"
    )  # Use the first row to count number of pipes
    result = []
    ...
    for line in merged_lines:
        ...
        if pipe_count < header_pipes:
            ...
            columns = line.split("|")[
                : header_pipes + 1
            ]  # +1 to keep last pipe at the end
            line = "|".join(columns)
...
def write_llm_output_and_logs(
    ...
    if first_run:
        ...
        batch_out_file_part = (
            f"{out_file_part}_combined_batch_{latest_batch_completed + 1}"
        )
        batch_part = f"Rows {start_row_reported} to {end_row}: "
    ...
    with open(
        whole_conversation_path_meta, "w", encoding="utf-8", errors="replace"
    ) as f:
    ...
    try:
        topic_with_response_df = pd.read_html(html_buffer)[
            0
    ...
    for index, row in topic_with_response_df.iterrows():
        references = re.split(
            r",\s*|\s+", str(row.iloc[4])
        )  # Split the reference numbers
        topic = row.iloc[0]
        subtopic = row.iloc[1]
        ...
        .agg(
            {
                ...
                "Summary": lambda x: "<br>".join(
                    sorted(
                        set(x),
                        key=lambda summary: out_reference_df.loc[
                            out_reference_df["Summary"] == summary, "Start row of group"
                        ].min(),
                    )
                ),
            }
    ...
    new_unique_topics_df = topic_with_response_df[
        ["General Topic", "Subtopic", "Sentiment"]
    ]  # , "Summary"

    # Join existing and new unique topics
    out_unique_topics_df = (
        pd.concat([new_unique_topics_df, existing_topics_df])
    ...
    out_unique_topics_df = out_unique_topics_df.merge(
...
def llm_query(
    file_data: pd.DataFrame,
    existing_topics_w_references_table: pd.DataFrame,
    existing_reference_df: pd.DataFrame,
    ...
    summarise_system_prompt: str = summarise_system_prompt,
    ...
    if file_data.empty:
        ...
        return (
            out_message,
            existing_topics_w_references_table,
    ...
    if (
        model_choice == "anthropic.claude-3-sonnet-20240229-v1:0"
        and file_data.shape[1] > 300
    ):
    ...
    if latest_batch_completed >= 1 or candidate_topics:
        ...
        if candidate_topics:
            ...
            if len(zero_shot_topics_series) > 120:
                print(
                    "Maximum 120 topics allowed to fit within large language model context limits."
                )
        ...
        if candidate_topics and existing_unique_topics_df.empty:
            existing_unique_topics_df = pd.DataFrame(
                data={
                    "General Topic": "",
                    "Subtopic": zero_shot_topics_list,
                    "Sentiment": "",
                }
            )
        ...
        # This part concatenates all zero shot and new topics together, so that for the next prompt the LLM will have the full list available
            ...
            existing_unique_topics_df = pd.concat(
                [existing_unique_topics_df, zero_shot_topics_df]
            ).drop_duplicates("Subtopic")

            # print("Full topics list with zero shot_dropped:", existing_unique_topics_df)

            existing_unique_topics_df.to_csv(
                output_folder + "Existing topics with zero shot dropped.csv"
            )

        unique_topics_markdown = (
            existing_unique_topics_df[["General Topic", "Subtopic", "Sentiment"]]
        ...
        existing_unique_topics_df.to_csv(
            output_folder
            + f"{out_file_part}_master_all_topic_tables_df_merged_"
            + model_choice_clean
            + "_temp_"
            + str(temperature)
            + "_batch_"
        ...
        formatted_summary_prompt = summarise_prompt.format(
        ...
        try:
            with open(
                formatted_prompt_output_path, "w", encoding="utf-8", errors="replace"
            ) as f:
        ...
        (
            new_topic_table_out_path,
            new_reference_table_out_path,
            new_unique_topics_df_out_path,
            new_topic_df,
            new_markdown_table,
            new_reference_df,
            new_unique_topics_df,
            master_batch_out_file_part,
            is_error,
        ...
        try:
            new_final_table_output_path = (
                output_folder
                + master_batch_out_file_part
                + "_full_final_response_"
                + model_choice_clean
                + "_temp_"
                + str(temperature)
                + ".txt"
            ...
            with open(
                new_final_table_output_path, "w", encoding="utf-8", errors="replace"
            ) as f:
    ...
    # If this is the first batch, run this
        ...
        if prompt3:
            ...
            formatted_prompt3 = prompt3

        batch_prompts = [formatted_prompt1, formatted_prompt2, formatted_prompt3][
            :number_of_requests
        ]  # Adjust this list to send fewer requests
        ...
        new_unique_topics_df = pd.concat(
            [new_unique_topics_df, existing_unique_topics_df]
        ).drop_duplicates("Subtopic")
        ...
        try:
            ...
            with open(
                final_table_output_path, "w", encoding="utf-8", errors="replace"
            ) as f:
